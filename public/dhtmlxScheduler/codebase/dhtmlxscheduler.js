...
//CHANGES CAN BE FOUND BY 'arkadiy'
...
scheduler.init = function (a, b, c) {
	...
	(function () {
		...
		dhtmlxEvent(window, "resize", function () {
			var g = a();
			if (!b(c, g))
				window.clearTimeout(scheduler._resize_timer), scheduler._resize_timer = window.setTimeout(function () {
	...
	buttons : {
		"delete" : function (a) {
			...
			scheduler._dhtmlx_confirm(b, scheduler.locale.labels.title_confirm_deleting, function () {
                ...
                var url = "/event/" + a + "/services/delete";//arkadiy                
                $.get(url);//arkadiy
			})
		},
		edit : function (a) {
		...
		details : function (a) {
            //alert(a);
			//alert("edit clicked");
            //window.location.href="/source/calendar/event/" + a; //arkadiy
            window.location.href="/event/" + a; //arkadiy
...
scheduler.render_event = function (a) {
	...
	if (!(a._sday < 0)) {
		...
		if (d) {
			...
			i = Math.max(scheduler.xy.min_event_height, (f - e) * this.config.hour_size_px /
					60),
		...
		if (b && b.className)
			...
			case "dhx_delete_btn":
				...
				scheduler._dhtmlx_confirm(c, scheduler.locale.labels.title_confirm_deleting, function () {           
					scheduler.deleteEvent(scheduler._lightbox_id);//arkadiy
                    var url = "/event/" + scheduler._lightbox_id + "/services/delete";//arkadiy                     
                    $.get(url);//arkadiy                    
			...
			if (scheduler.config.fix_tab_position)
				for (var c = scheduler._els.dhx_cal_navline[0].getElementsByTagName("div"),
					...
					if (n)
						...
						default:
							l.style.left = e + "px",
							l.className += " dhx_cal_tab_standalone",
							e = e + 14 + l.offsetWidth
						}
				}
		});
		scheduler._skin_init = function () {}
