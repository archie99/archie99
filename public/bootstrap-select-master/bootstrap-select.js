	...
	Selectpicker.prototype = {
		...
		createLi : function () {
			...
			this.$element.find('option').each(function () {
				...
				var id = $this.attr('id'); //my addition
				//alert(id);
				...
				if (that.options.hideDisabled && ($this.is(':disabled') || $this.parent().is(':disabled'))) {
					...
					if ($this.index() === 0) {
						...
						if ($this[0].index !== 0) {
							_liA.push(
								...
								'<dt>' + label + '</dt>' +
								that.createA(text, 'opt ' + optionClass, inline, id));
						} else {
							_liA.push(
								'<dt>' + label + '</dt>' +
								that.createA(text, 'opt ' + optionClass, inline, id));
						...
						_liA.push(that.createA(text, 'opt ' + optionClass, inline, id));
					}
					...
					_liA.push(that.createA(text, optionClass, inline, id));
				}
			});

			$.each(_liA, function (i, item) {
				_liHtml += '<li rel=' + i + '>' + item + '</li>';
				//_liHtml += '<li id=' + id rel=' + i + '>' + item + '</li>'; //my addition
			});
		...
		createA : function (text, classes, inline, id) {
			return '<a id="' + id +'" tabindex="0" class="' + classes + '" style="' + inline + '">' +
			text +
